
WITH recent_findings AS (
    SELECT 
        vf.id,
        vf.added,
        wl.first_seen,
        DATE_PART('day', vf.added - wl.first_seen) AS days_old,
        c.name AS cve_id,
        i.name AS image_name,
        r.name AS container_resource_name,
        cl.name AS cluster_name
    FROM 
        cloudplatform_vulnerabilityfinding vf
    LEFT JOIN cloudplatform_vulnerabilityfindingworkload vfw ON vf.id = vfw.vulnerability_finding_id
    LEFT JOIN cloudplatform_workload wl ON vfw.workload_id = wl.id
    LEFT JOIN cloudplatform_resource r ON wl.resource_id = r.id
    LEFT JOIN cloudplatform_resource cl ON r.parent_id = cl.id AND cl.type = 'CL'
    LEFT JOIN cloudplatform_image i ON wl.image_id = i.id
    JOIN cloudplatform_vulnerability v ON vf.vulnerability_id = v.id
    JOIN cloudplatform_vulnerabilitysource vs ON vs.id = v.source_id
    JOIN cloudplatform_vulnerabilitysourcecve vsc ON vsc.vulnerability_source_id = vs.id
    JOIN cloudplatform_cve c ON c.id = vsc.cve_id
    JOIN acti_stix_vulns acti ON c.name = acti.name
    WHERE 
        vf.added >= CURRENT_DATE - INTERVAL '4 days'
        AND (wl.first_seen IS NULL OR wl.first_seen < CURRENT_DATE - INTERVAL '4 days')
        AND r.provider = 'OP'
        AND r.type = 'CN'
),
previous_findings AS (
    SELECT DISTINCT 
        c.name AS cve_id,
        i.name AS image_name
    FROM 
        cloudplatform_vulnerabilityfinding vf
    LEFT JOIN cloudplatform_vulnerabilityfindingworkload vfw ON vf.id = vfw.vulnerability_finding_id
    LEFT JOIN cloudplatform_workload wl ON vfw.workload_id = wl.id
    LEFT JOIN cloudplatform_resource r ON wl.resource_id = r.id
    LEFT JOIN cloudplatform_image i ON wl.image_id = i.id
    JOIN cloudplatform_vulnerability v ON vf.vulnerability_id = v.id
    JOIN cloudplatform_vulnerabilitysource vs ON vs.id = v.source_id
    JOIN cloudplatform_vulnerabilitysourcecve vsc ON vsc.vulnerability_source_id = vs.id
    JOIN cloudplatform_cve c ON c.id = vsc.cve_id
    WHERE 
        vf.added < CURRENT_DATE - INTERVAL '4 days'
        AND r.provider = 'OP'
        AND r.type = 'CN'
)
SELECT *
FROM recent_findings rf
WHERE NOT EXISTS (
    SELECT 1 
    FROM previous_findings pf
    WHERE rf.cve_id = pf.cve_id
      AND rf.image_name = pf.image_name
)
ORDER BY rf.added DESC;
